...
{
    public partial class SaleCash : Form
    {
        public SaleCash()
        {
            InitializeComponent();
        ...
        private void SaleCash_Load(object sender, EventArgs e)
        {
        ...
        private void grdShow()
        {
            ...
            string sql = "SELECT sdate as SaleDate, billno as BillNumber, tot  as Total, bal  as Balance FROM sale where cname like'" + sname + "'";
            SqlConnection connection = new SqlConnection(connetionString);
            SqlDataAdapter dataadapter = new SqlDataAdapter(sql, connection);
            DataSet ds = new DataSet();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    pno = (string)rows[0].Cells["BillNumber"].Value.ToString();
                ...
                if (sname == true & tra == true)
                {
                    if (byc == true | byck == true)
                    {
                        ...
                        string sql = "insert into salecash(billno,method,paydate,amt,bal,cqno,cqdate,bname) values (" + pno + ", '" + method + "', '" + pdate + "', " + pmt + "," + bal1 + ", '" + chqno + "' , '" + cqdate + "' , '" + bname + "');";
                        // MessageBox.Show(sql);

                        string sqlup = "update sale set bal=" + b + " where billno=" + pno;
